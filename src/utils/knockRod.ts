...
export class KnockRod extends DocumentFragment {
    ...
    public async setSafetySpeed(enabled: boolean): Promise<void> {
        ...
        try {
            ...
            console.info("response: " +
                KnockRod.toHex(await this.readBytes(setSafetySpeedEnabled(enabled).byteLength)));
        } finally {
            release();
        }
    }
    ...
    public async home(): Promise<void> {
        ...
        try {
            await this.waitUntil(12000, () => this.state && this.state.deviceStatusRegister1.has(DSS1.HEND) || false);
        } catch (e) {
    ...
    public addEventListener<K extends keyof ThrusterEventHandlersEventMap>(
        type: K,
        listener: ((evt: ThrusterEventHandlersEventMap[K]) => any) | EventListenerObject | null,
    ...
    private updateState(updater: (old: KnockRodState) => KnockRodState) {
        ...
        this.dispatchEvent(new CustomEvent(
            "stateChange",
            {detail: {oldState, state: this._state}}) as StateChangeEvent);
    }
     ...
     */
    private async queryAwaitResponseWithRetry<T>(query: ArrayBufferLike,
                                                 responseSize: number,
                                                 responseExtractor: (a: Uint8Array) => T | undefined,
                                                 retryCount: number | undefined,
                                                 retryInterval: number | undefined): Promise<T> {
        ...
        for (let i = 0; i < actualRetryCount; ++i) {
            ...
            try {
                ...
                const readResponse = await Promise.any([this.readBytes(responseSize),
